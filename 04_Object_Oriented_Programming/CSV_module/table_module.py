class Table:
    ...
    def get_rows(self, row_numbers):
        return Table({col: [self.table[col][i] for i in row_numbers] for col in self.columns})

    def get_columns(self, column_names):
        return Table({col: self.table[col] for col in column_names})

    def get_value(self, row_number, col_name):
        return self.table[col_name][row_number]
...
if __name__ == "__main__":
    ...
    pokedex.show()
    stats = pokedex.get_columns(["species_ID", "HP", "phAtk", "phDef"])
    print("Stats size = ", stats.get_size())
